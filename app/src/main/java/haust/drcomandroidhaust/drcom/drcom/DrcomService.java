...
import haust.drcomandroidhaust.drcom.MainActivity;
            ...
            while (reconnectTimes <= DrcomConfig.ReconnectTIMES){
                try {
                    ...
                    if(reconnectTimes > DrcomConfig.ReconnectTIMES){
                        performMsgCall("Give re-connect, Eixt");
                        break;
                    }
                    performMsgCall("Communication timeout"+reconnectTimes+"'s auto re-connect");
                    ...
                    if(reconnectTimes >= 2){
                        ...
                        if (!challenge(challengeTimes++)||!login()) {
                            performMsgCall("auto re-connect faild, exit");
                            break;
                        }
                    }
...
//                    notifyLogout();
            ...
            e.printStackTrace();
            performMsgCall("Communication timeout :"+e.getMessage());
            exception = true;
        } catch (IOException e) {
            performMsgCall("IO Exception"+e.getMessage());
...
//                cancelNotification();
    ...
    private void init()  {
        try {
            ...
            performMsgCall("Port conflict"+e.getMessage());
        } catch (UnknownHostException e) {
            performMsgCall("not a school wifi");
        }catch (Exception e){
//            cancelNotification();
            e.printStackTrace();
    ...
    private boolean challenge(int tryTimes) {
        try {
            ...
            performLogCall("Challenge timeout");
        } catch (IOException e) {
            performMsgCall("Authentication failed");
        }
        return true;
    }
            ...
            if (recv[0] == 0x05) {
                switch (recv[4]){
                    case 0x01:
                        ...
                        break;
                    case 0x03:
                        tip(R.string.info_error);
                        break;
//                    case 0x05:
//                        performMsgCall("账号没钱啦");
//                        break;
                    case 0x0B:
                        performMsgCall("MAC ERROR");
                        break;
                    default:
                        performMsgCall("UNKNOW ERROR");
                }
            } else {
                performMsgCall("UNKNOE NAME ERROR");
            }
            return false;
        }
...
//
//                    WifiUtil wifiUtils = new WifiUtil(context);
//                    boolean isSchoolWifi = WifiUtil.currentIsSchoolWifi(wifiUtils);
//                    if(!isSchoolWifi){
//                        performMsgCall("wifi changed and exit");
////                        cancelNotification();
//                        stopSelf(); //停止service
//                        return;
//                    }
...
//                    performMsgCall("wifi is closed and exit");
////                    cancelNotification();
//                    stopSelf(); //停止service
//                }
